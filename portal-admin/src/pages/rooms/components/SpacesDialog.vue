...
const spaceGroups = computed(() => {
  ...
  const groups = [
    { key: 'CHAT', label: 'Chat', items: spaces.filter(s => normalizeSpaceType(s.type) === 'CHAT') },
    { key: 'VOICE', label: 'Voice', items: spaces.filter(s => normalizeSpaceType(s.type) === 'VOICE') },
    { key: 'CALENDAR', label: 'Calendar', items: spaces.filter(s => normalizeSpaceType(s.type) === 'CALENDAR') },
    { key: 'NOTE', label: 'Note', items: spaces.filter(s => normalizeSpaceType(s.type) === 'NOTE') },
    { key: 'TASK', label: 'Task', items: spaces.filter(s => normalizeSpaceType(s.type) === 'TASK') },
  ]

  const groupedIds = new Set(groups.flatMap(g => g.items.map(s => s.id)))
  const ungrouped = spaces.filter(s => !groupedIds.has(s.id))
    ...
    <DialogContent class="max-w-[760px] gap-0 overflow-hidden p-0">
      <DialogHeader class="border-b border-border px-6 py-4">
        <DialogTitle class="text-[15px] font-semibold">
          Danh sách Spaces ({{ props.spaces.length }})
        </DialogTitle>
