<script setup lang="ts">
import type { Row } from '@tanstack/vue-table'
import type { Component } from 'vue'
import { EllipsisIcon } from '@lucide/vue'
import { Modal, ModalContent } from '@/components/prop-ui/modal'
import type { Room } from '../data/schema'

interface Props { row: Row<Room> }
const props = defineProps<Props>()
const room = computed(() => props.row.original)
const isOpen = ref(false)
const showComponent = shallowRef<Component | null>(null)
const detailRoomId = ref<string | null>(null)

type TCommand = 'edit' | 'delete' | 'detail'

const componentLoader: Record<TCommand, () => Promise<{ default: Component }>> = {
  detail: () => import('./room-detail.vue'),
  edit: () => import('./room-resource.vue'),
  delete: () => import('./room-delete.vue'),
}

async function handleSelect(command: TCommand) {
  try {
    const { default: component } = await componentLoader[command]()
    showComponent.value = component
    if (command === 'detail') detailRoomId.value = room.value.id
    isOpen.value = true
  } catch (e) {
    console.error(`Failed to load component for "${command}"`, e)
  }
}
</script>

<template>
  <Modal v-model:open="isOpen">
    <UiDropdownMenu>
      <UiDropdownMenuTrigger as-child>
        <UiButton variant="ghost" class="flex h-8 w-8 p-0 data-[state=open]:bg-muted">
          <EllipsisIcon class="size-4" />
          <span class="sr-only">Open menu</span>
        </UiButton>
      </UiDropdownMenuTrigger>
      <UiDropdownMenuContent align="end" class="w-[160px]">
        <UiDropdownMenuItem @click.stop="handleSelect('detail')">
          View Detail
        </UiDropdownMenuItem>
        <UiDropdownMenuItem @click.stop="handleSelect('edit')">
          Edit
        </UiDropdownMenuItem>
        <UiDropdownMenuSeparator />
        <UiDropdownMenuItem @click.stop="handleSelect('delete')" class="text-destructive">
          Delete
          <UiDropdownMenuShortcut>⌘⌫</UiDropdownMenuShortcut>
        </UiDropdownMenuItem>
      </UiDropdownMenuContent>
    </UiDropdownMenu>
    <ModalContent>
      <component
        :is="showComponent"
        :room="room"
        :room-id="detailRoomId"
        @close="isOpen = false"
        @remove="isOpen = false"
        @refresh="isOpen = false"
      />
    </ModalContent>
  </Modal>
</template> 