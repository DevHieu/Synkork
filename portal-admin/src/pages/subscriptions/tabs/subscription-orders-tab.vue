...
const selectedBillingCycle = ref('ALL')
...
function buildParams(): InvoiceSearchParams {
  ...
  if (selectedBillingCycle.value !== 'ALL')
    params.billingCycle = selectedBillingCycle.value
...
const hasActiveFilter = computed(() =>
  ...
  || (selectedBillingCycle.value !== 'ALL')
...
watch([debounceSearchKeyword, selectedStatus, selectedPlan, selectedBillingCycle, selectedPaymentMethod, dateRange], () => {
  currentPage.value = 1
  fetchInvoices()
})
...
onMounted(() => {
  const initialKeyword = props.keyword || keywordParam
  if (initialKeyword !== '') {
...
const columns = computed<TableColumn<Invoice>[]>(() => [
  ...
  },
  {
    header: 'Chu kỳ',
    minWidth: 110,
    render: row => row.billingCycle === 'YEARLY' ? 'Năm' : row.billingCycle === 'MONTHLY' ? 'Tháng' : 'N/A',
  },
  {
...
<template>
  <div class="space-y-4">
    <div class="flex flex-wrap items-center gap-3">
      ...
      <div class="w-[160px]">
        <UiSelect v-model="selectedBillingCycle">
          <SelectTrigger class="h-9 w-full">
            <SelectValue placeholder="Chu kỳ" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem v-for="item in billingCycleOptions" :key="item.value" :value="item.value">
              {{ item.label }}
            </SelectItem>
          </SelectContent>
        </UiSelect>
