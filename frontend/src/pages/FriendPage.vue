    ...
    <div class="h-12 border-b border-border flex items-center px-4 gap-4 flex-shrink-0">
      ...
      <div class="flex gap-1 bg-muted rounded-md p-0.5">
        ...
        <button
          ...
          :class="activeTab === 'pending' ? 'bg-card text-foreground' : 'text-muted-foreground hover:text-foreground'"
          class="px-5 py-1.5 text-sm font-medium rounded transition flex items-center gap-1.5"
        >
          Đang chờ xử lý
          <span class="bg-red-500 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">
            {{ totalPending() }}
          </span>
        </button>
