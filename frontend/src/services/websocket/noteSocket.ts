...
export const noteSocket = {
    subscribeCreateNote(spaceId: string, callback: (payload: any) => void) {
        subscriptions[`create_${spaceId}`] = socketService.subscribe(`/topic/space/${spaceId}/notes/create`, callback)
    },

    subscribeUpdateNote(spaceId: string, callback: (payload: any) => void) {
        subscriptions[`update_${spaceId}`] = socketService.subscribe(`/topic/space/${spaceId}/notes/update`, callback)
    },

    subscribeDeleteNote(spaceId: string, callback: (payload: any) => void) {
        subscriptions[`delete_${spaceId}`] = socketService.subscribe(`/topic/space/${spaceId}/notes/delete`, callback)
    },
    ...
    unsubscribeAll(spaceId: string) {
        ;[`create`, `update`, `delete`, `pin`].forEach(type => {
