...
<template>
  <div class="theme-root">
    ...
    <section class="ts-section">
      ...
      <div class="swatch-grid">
        <button v-for="opt in pastelThemes" :key="opt.id"
    ...
    <section class="ts-section">
      ...
      <div class="swatch-grid">
        <button v-for="opt in ombreThemes" :key="opt.id"
...
.ts-section-header { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.85rem; }
.ts-section-icon { width: 13px; height: 13px; }
.ts-label-small { font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: var(--muted-foreground); margin-bottom: 0.75rem; }
.group-badge { font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; padding: 0.18rem 0.6rem; border-radius: 999px; }
...
.ts-divider { height: 1px; background: var(--border); opacity: 0.4; margin: 1.25rem 0; }
.mode-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.6rem; }
.mode-card { position: relative; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.45rem; padding: 1rem 0.5rem; border-radius: 8px; border: 1.5px solid var(--border); background: var(--muted); cursor: pointer; transition: all 0.15s; }
...
.mode-check { position: absolute; top: 6px; right: 6px; width: 18px; height: 18px; border-radius: 50%; background: var(--primary); display: flex; align-items: center; justify-content: center; }
.check-icon { width: 11px; height: 11px; color: white; }
.swatch-grid { display: flex; flex-wrap: wrap; gap: 0.55rem; }
.swatch-btn { width: 36px; height: 36px; border-radius: 50%; border: 2.5px solid transparent; cursor: pointer; position: relative; display: flex; align-items: center; justify-content: center; transition: transform 0.12s, border-color 0.15s, box-shadow 0.15s; flex-shrink: 0; }
.swatch-btn:hover { transform: scale(1.18); }
.swatch-btn.selected { border-color: var(--foreground); box-shadow: 0 0 0 2px var(--background), 0 0 0 4px var(--foreground); }
.pastel-swatch { width: 34px; height: 34px; }
.ombre-swatch { width: 52px; height: 26px; border-radius: 999px; }
.swatch-check { width: 14px; height: 14px; color: white; filter: drop-shadow(0 1px 1px rgba(0,0,0,.5)); }
.selected-label { display: flex; align-items: center; gap: 0.45rem; margin-top: 0.65rem; font-size: 0.8rem; color: var(--muted-foreground); }
.dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
.ombre-tag { font-weight: 700; color: var(--primary); }
.preview-card { background: var(--muted); border-radius: 10px; padding: 1rem; border: 1px solid var(--border); }
.preview-header { display: flex; align-items: center; gap: 0.6rem; padding: 0.5rem 0.75rem; background: var(--accent); border-radius: 6px; margin-bottom: 0.85rem; }
.preview-avatar { width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.8rem; font-weight: 700; color: white; flex-shrink: 0; }
...
.preview-bubble { max-width: 70%; padding: 0.4rem 0.7rem; font-size: 0.8rem; color: var(--foreground); background: var(--accent); border-radius: 8px; }
.preview-bubble.self { color: white; margin-left: auto; }
.preview-btn-row { margin-top: 0.75rem; }
.preview-btn { padding: 0.4rem 1rem; font-size: 0.78rem; color: white; font-weight: 600; border: none; border-radius: 6px; cursor: default; }
.action-row { display: flex; align-items: center; gap: 0.75rem; margin-top: 1.5rem; }
.reset-btn { display: flex; align-items: center; gap: 0.35rem; padding: 0.42rem 0.85rem; border-radius: 6px; font-size: 0.8rem; border: 1px solid var(--border); background: var(--accent); color: var(--muted-foreground); cursor: pointer; transition: all 0.12s; }
