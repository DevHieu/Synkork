...
// Module-level: tồn tại suốt vòng đời app, không reset khi unmount
let _initDone = false
const _shared = reactive({ themeId: "default" as ThemeId, mode: "dark" as ThemeMode })
...
// ── Component state (khởi tạo từ shared để giữ live preview) ─────
const themeMode     = ref<ThemeMode>(_shared.mode)
const selectedTheme = ref<ThemeId>(_shared.themeId)
...
watch([selectedTheme, themeMode], ([tId, mode]) => {
  _shared.themeId = tId
  _shared.mode    = mode as ThemeMode
  applyToDom(tId, mode as ThemeMode)
})
...
onMounted(() => {
  if (!_initDone) {
    ...
    _shared.themeId  = savedTheme ?? "default"
    _shared.mode     = savedMode  ?? "dark"
    _initDone        = true
  }
  // Sync về shared — giữ lại live preview nếu đã chọn trước khi switch tab
  selectedTheme.value = _shared.themeId
  themeMode.value     = _shared.mode
  applyToDom(selectedTheme.value, themeMode.value)
})
  ...
  <div class="theme-root">
    ...
    <section class="ts-section">
      ...
      <div class="mode-grid">
        <button
          ...
          :key="m.value"
          class="mode-card"
          :class="{ selected: themeMode === m.value }"
    ...
    <section class="ts-section">
      ...
      <div class="swatch-grid">
        <button
          v-for="opt in normalThemes" :key="opt.id"
          class="swatch-btn" :class="{ selected: selectedTheme === opt.id }"
          :title="opt.label" :style="swatchStyle(opt)"
    ...
    <section class="ts-section">
      ...
      <div class="swatch-grid">
        <button
          v-for="opt in pastelThemes" :key="opt.id"
          class="swatch-btn pastel-swatch" :class="{ selected: selectedTheme === opt.id }"
          :title="opt.label" :style="swatchStyle(opt)"
          @click="selectedTheme = opt.id"
        >
          <Check v-if="selectedTheme === opt.id" class="swatch-check" />
    ...
    <section class="ts-section">
      ...
      <div class="swatch-grid">
        <button
          v-for="opt in ombreThemes" :key="opt.id"
          class="swatch-btn ombre-swatch" :class="{ selected: selectedTheme === opt.id }"
          :title="opt.label" :style="swatchStyle(opt)"
          @click="selectedTheme = opt.id"
        >
          <Check v-if="selectedTheme === opt.id" class="swatch-check" />
...
.ts-label-small {
  ...
  letter-spacing: 0.07em; color: var(--muted-foreground); margin-bottom: 0.75rem;
}

.group-badge {
  font-size: 0.72rem; font-weight: 700; text-transform: uppercase;
  letter-spacing: 0.07em; padding: 0.18rem 0.6rem; border-radius: 999px;
}
...
.mode-card {
  ...
  padding: 1rem 0.5rem; border-radius: 8px; border: 1.5px solid var(--border);
...
.swatch-btn {
  ...
  transition: transform 0.12s, border-color 0.15s, box-shadow 0.15s; flex-shrink: 0;
}
...
.selected-label {
  ...
  margin-top: 0.65rem; font-size: 0.8rem; color: var(--muted-foreground);
}
...
.preview-header {
  ...
  padding: 0.5rem 0.75rem; background: var(--accent);
...
.preview-bubble {
  max-width: 70%; padding: 0.4rem 0.7rem; font-size: 0.8rem;
  color: var(--foreground); background: var(--accent); border-radius: 8px;
}
.preview-bubble.self { color: white; margin-left: auto; }
.preview-btn-row { margin-top: 0.75rem; }
.preview-btn {
  padding: 0.4rem 1rem; font-size: 0.78rem; color: white;
  font-weight: 600; border: none; border-radius: 6px; cursor: default;
}
...
.reset-btn {
  ...
  padding: 0.42rem 0.85rem; border-radius: 6px; font-size: 0.8rem;
...
.apply-btn {
  margin-left: auto; padding: 0.45rem 1.25rem; border-radius: 6px;
  font-size: 0.84rem; font-weight: 700; background: var(--primary);
  color: white; cursor: pointer; transition: opacity 0.12s; border: none;
}
