    ...
    <Transition name="sf">
      <div class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/75" @click.self="emit('close')">
        <div class="flex w-[min(960px,96vw)] h-[min(660px,95vh)] overflow-hidden rounded-lg shadow-2xl">
          ...
          <!-- ═══ LEFT SIDEBAR ═══ -->
          <div class="w-[238px] min-w-[238px] flex flex-col overflow-y-auto pt-14 bg-muted">
            ...
            <div class="px-4 pb-3">
              <div class="relative cursor-pointer group/ava w-fit">
                ...
                <div class="absolute inset-0 rounded-full flex items-center justify-center bg-black/50 opacity-0 group-hover/ava:opacity-100 transition-opacity">
                  <Pencil class="size-4 text-white" />
                </div>
              </div>
              <div class="mt-2">
                <p class="text-sm font-bold text-foreground">{{ displayName }}</p>
                <button class="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors mt-0.5">
          ...
          <div class="flex-1 flex flex-col bg-card overflow-hidden">
            ...
            <div class="flex items-center justify-between px-7 pt-5 pb-3 shrink-0">
              <h2 class="text-base font-bold text-foreground">
                ...
                <template v-else>{{ settingsTabs.flatMap(g => g.items).find(i => i.id === activeTab)?.label }}</template>
              </h2>
              <Button variant="ghost" size="sm" class="gap-1.5 text-muted-foreground text-[11px]" @click="emit('close')">
                <X class="size-[18px]" />
            ...
            <div class="flex-1 overflow-y-auto px-8 pb-12 scrollbar-thin scrollbar-thumb-border">
              ...
              <template v-if="activeTab === 'account'">
                ...
                <div class="rounded-xl overflow-hidden bg-muted mb-5">
                  ...
                  <div class="flex items-end justify-between px-4 -mt-10 mb-3">
                    <div class="relative cursor-pointer group/ava2">
                      ...
                      <div class="absolute inset-0 rounded-full flex items-center justify-center bg-black/50 opacity-0 group-hover/ava2:opacity-100 transition-opacity">
                ...
                <Transition name="toast">
                  <div v-if="editSuccess" class="flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium mb-4 bg-primary/15 text-primary">
                ...
                <div class="flex border-b border-border mb-5">
                  <button class="px-4 py-2 text-sm font-medium text-primary border-b-2 border-primary -mb-px transition-colors">Bảo mật</button>
                  <button class="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground -mb-px transition-colors">Trạng thái</button>
                </div>
                ...
                <div class="rounded-lg overflow-hidden bg-muted">
                  ...
                  <div class="flex items-start justify-between gap-4 px-4 py-3">
                    <div class="flex-1 min-w-0">
                      ...
                      <template v-if="editingField === 'displayName'">
                        ...
                        <div class="flex gap-2 mt-2">
                          <Button size="sm" class="h-7 text-xs" :disabled="editLoading" @click="saveEdit('displayName')">
                            {{ editLoading ? '...' : 'Lưu' }}
                          </Button>
                    ...
                    <Button v-if="editingField !== 'displayName'" variant="outline" size="sm" class="mt-5 text-xs shrink-0" @click="startEdit('displayName')">
                      Chỉnh sửa
                    </Button>
                  </div>
                  ...
                  <div class="flex items-start justify-between gap-4 px-4 py-3">
                    ...
                    <Button v-if="editingField !== 'username'" variant="outline" size="sm" class="mt-5 text-xs shrink-0" @click="startEdit('username')">
                      Chỉnh sửa
                    </Button>
                  </div>
                ...
                <!-- ══ PASSWORD SECTION ══ -->
                <div class="mt-7">
                  <p class="text-[10px] uppercase tracking-widest font-bold text-muted-foreground mb-3">Mật Khẩu & Xác Thực</p>
                    ...
                    <div class="rounded-lg bg-muted p-4 space-y-4">
                      ...
                      <div class="flex items-start gap-3 p-3 rounded-lg border border-primary/20 bg-primary/8">
                        <div class="shrink-0 w-9 h-9 rounded-full bg-background flex items-center justify-center">
                          ...
                          <!-- GitHub -->
                          <svg v-else-if="currentUser?.provider === 'GITHUB'" viewBox="0 0 24 24" class="size-5" fill="currentColor">
                      ...
                      <div class="space-y-3">
                        ...
                        <div class="space-y-1.5">
                          <Label class="text-[10px] uppercase tracking-wider text-muted-foreground">Xác nhận mật khẩu</Label>
                          <div class="relative">
                      ...
                      <Transition name="toast">
                        <div v-if="createPwError" class="flex items-center gap-2 px-3 py-2 rounded-md text-xs font-medium bg-destructive/10 text-destructive">
                          <AlertCircle class="size-4 shrink-0" /> {{ createPwError }}
                        </div>
                      </Transition>
                      <Transition name="toast">
                        <div v-if="createPwSuccess" class="flex items-center gap-2 px-3 py-2 rounded-md text-xs font-medium bg-primary/15 text-primary">
                      ...
                      <Button
                        class="text-sm"
                        :disabled="createPwLoading || !createPwForm.next || !createPwForm.confirm"
                  ...
                  <template v-else>
                    <div class="rounded-lg bg-muted p-4 space-y-3">
                      <div class="space-y-1.5">
                        <Label class="text-[10px] uppercase tracking-wider text-muted-foreground">Mật khẩu hiện tại</Label>
                        <div class="relative">
                          ...
                          <button class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors"
                            ...
                            <Eye v-if="!showPw.current" class="size-4" /><EyeOff v-else class="size-4" />
                          </button>
                        </div>
                      </div>
                      <div class="space-y-1.5">
                        ...
                        <div class="relative">
                          ...
                          <button class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors"
                            ...
                            <Eye v-if="!showPw.next" class="size-4" /><EyeOff v-else class="size-4" />
                          </button>
                        </div>
                      </div>
                      <div class="space-y-1.5">
                        <Label class="text-[10px] uppercase tracking-wider text-muted-foreground">Xác nhận mật khẩu mới</Label>
                        <div class="relative">
                          <Input :type="showPw.confirm ? 'text' : 'password'" v-model="pwForm.confirm"
                            class="pr-10 h-9 text-sm" placeholder="••••••••" />
                          <button class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors"
                            @click="showPw.confirm = !showPw.confirm">
                            <Eye v-if="!showPw.confirm" class="size-4" /><EyeOff v-else class="size-4" />
                          </button>
                        </div>
                      </div>

                      <Transition name="toast">
                        <div v-if="pwError" class="flex items-center gap-2 px-3 py-2 rounded-md text-xs font-medium bg-destructive/10 text-destructive">
                          <AlertCircle class="size-4 shrink-0" /> {{ pwError }}
                        </div>
                      </Transition>
                      <Transition name="toast">
                        <div v-if="pwSuccess" class="flex items-center gap-2 px-3 py-2 rounded-md text-xs font-medium bg-primary/15 text-primary">
              ...
              <template v-else-if="activeTab === 'billing'">
                <p class="text-[10px] uppercase tracking-widest font-bold text-muted-foreground mb-3">Nâng cấp lên Pro</p>
...
.sf-enter-active, .sf-leave-active { transition: opacity 0.18s ease; }
