...
<template>
  <SidebarMenu>
    <SidebarMenuItem>
      <SidebarMenuButton
        size="lg"
        class="hover:bg-transparent active:bg-transparent group/navuser"
        :class="collapsed ? 'h-16.5 w-12 justify-center px-1 mx-auto' : 'h-16.5'"
      >
        <!-- Avatar -->
        <div class="h-fit rounded-full p-1 hover:bg-sidebar-accent/50 active:bg-sidebar-accent/90 transition-colors duration-200">
        ...
        <template v-if="!collapsed">
          ...
          <button
            ...
            class="ml-auto p-1.5 rounded-md opacity-0 group-hover/navuser:opacity-100
                   hover:bg-sidebar-accent/60 transition-all duration-200
        ...
        <template v-else>
          ...
          <button
            @click.stop="showSettings = true"
            class="absolute bottom-1 right-1 p-0.5 opacity-0 group-hover/navuser:opacity-100
                   transition-all duration-200 text-sidebar-foreground/60 hover:text-sidebar-foreground"
            title="Cài đặt"
          >
