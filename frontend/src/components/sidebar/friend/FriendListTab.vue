      ...
      <div v-else-if="filteredFriends.length > 0">
        ...
        >
          ...
          <div class="relative w-10 h-10 flex-shrink-0">
            ...
            <div class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-card bg-muted-foreground" />
          </div>
          ...
          <div class="opacity-0 group-hover:opacity-100 flex gap-1 items-center" @click.stop>
            ...
            <button
              class="w-8 h-8 hover:bg-muted rounded flex items-center justify-center"
              title="Nhắn tin"
              @click="openChat(f)"
            >
              💬
            ...
            <!-- Nút ⋮ dọc -->
            <button
              class="w-8 h-8 hover:bg-muted rounded flex items-center justify-center text-muted-foreground"
              title="Thêm"
          ...
          >
            ...
            <button
              class="w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-muted text-foreground transition"
          ...
          <!-- Xác nhận xóa -->
          <div
            v-if="confirmRemoveId === f.id"
            class="absolute right-2 top-12 z-50 min-w-[200px] rounded-md border border-border bg-popover shadow-lg p-3"
            @click.stop
          >
            <p class="text-sm mb-3">Xóa <span class="font-semibold">{{ f.name }}</span> khỏi danh sách bạn bè?</p>
            <div class="flex gap-2 justify-end">
              <button
                class="text-xs px-3 py-1.5 rounded bg-muted hover:bg-muted/80 text-muted-foreground transition"
