...
<template>
  ...
  >
    ...
    >
      <Mic v-if="micOn" class="h-5 w-5" /><MicOff v-else class="h-5 w-5" />
      {{ micOn ? "Mic" : "Mic off" }}
    </button>
    <button
      @click="() => toggleVideo()"
    ...
    >
      <Video v-if="videoOn" class="h-5 w-5" /><VideoOff
        v-else
        class="h-5 w-5"
      />
      {{ videoOn ? "Cam" : "Cam off" }}
    ...
    >
      <Volume2 v-if="audioOn" class="h-5 w-5" /><VolumeX
        v-else
        class="h-5 w-5"
      />
    ...
    <button
      @click="voiceSpaceStore.leaveRoom()"
