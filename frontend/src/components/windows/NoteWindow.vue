    ...
    <header class="sticky top-0 z-40 border-b bg-background/95 backdrop-blur">
      ...
      <div class="max-w-6xl mx-auto px-4 h-14 flex items-center gap-4">
        ...
        <div class="ml-auto flex items-center gap-3">
          ...
          <span class="text-xs hidden sm:block">{{ store.notes.length }} ghi chú</span>
      ...
      <template v-else>
        ...
        <div v-if="store.filteredNotes.length === 0 && !store.loading" class="text-center py-20">
          ...
          <p class="text-sm text-muted-foreground">
            {{ store.searchQuery ? 'Không tìm thấy ghi chú nào' : 'Chưa có ghi chú nào' }}
          </p>
        ...
        <template v-else>
          ...
          <section v-if="store.unpinnedNotes.length > 0">
            <h2 v-if="store.pinnedNotes.length > 0" class="text-xs mb-3 text-muted-foreground font-semibold uppercase">
              Khác
            </h2>
            <GridLayout v-model:layout="layout" :col-num="12" :row-height="100" :is-draggable="true"
              ...
              <GridItem v-for="item in layout" :key="item.i" :x="item.x" :y="item.y" :w="item.w" :h="item.h" :i="item.i"
                ...
                <NoteCard v-if="getNoteById(item.i)" :note="getNoteById(item.i)!" @view="openDetail" @edit="openEdit"
                  @delete="confirmDelete" @pin="handleTogglePin" @reminder="openReminder" />
              </GridItem>
            </GridLayout>
          </section>
...
import { NotebookPen, Plus, Search, X, Pin, Loader2, AlertCircle, Bell } from 'lucide-vue-next'
...
watch(
  ...
  (space, prevSpace) => {
    if (!space?.id) return
    if (space.id === prevSpace?.id) return
    if (prevSpace?.id) store.disconnectSocket(prevSpace.id)
    store.notes = []
    layout.value = []
    layoutUpdateCount = 0
    store.fetchNotes(space.id)
  },
  { immediate: true }
)
...
function openCreate() { selectedNote.value = null; dialogOpen.value = true }
function openDetail(note: Note) { selectedNote.value = note; detailOpen.value = true }
function openEdit(note: Note) { detailOpen.value = false; selectedNote.value = note; dialogOpen.value = true }
function confirmDelete(id: string) { deleteTargetId.value = id; detailOpen.value = false; confirmOpen.value = true }
...
function openReminder(note: Note) { reminderNote.value = note; reminderOpen.value = true }
...
async function handleSnooze(note: Note) {
  ...
  await store.setNoteReminder(spaceId.value, note.id, next.toISOString())
