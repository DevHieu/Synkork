...
<template>
  <div class="shrink-0 flex gap-2 overflow-x-auto no-scrollbar" style="height: 96px">
    <div v-for="tile in otherPeople" :key="tile.id"
      ...
      style="aspect-ratio: 16/9" @click="emit('focus', tile.id)">
      <div v-if="tile.type === 'participant' && !tile.videoOn" class="w-full h-full flex items-center justify-center">
