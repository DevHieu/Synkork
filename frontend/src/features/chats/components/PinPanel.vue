...
<template>
  <aside class="hidden lg:flex flex-col w-full h-full shrink-0 overflow-hidden" style="
    ...
    <div class="px-3 pt-4 pb-2 flex items-center justify-between">
      <div class="flex items-center gap-1.5 text-[11px] font-medium tracking-widest uppercase"
    ...
    <div ref="scrollContainer" class="flex-1 overflow-y-auto pb-4">
      <!-- Loading ban đầu -->
      <div v-if="pinLoading && !pinnedMessages.length" class="px-2 py-1 space-y-2">
        <div v-for="i in 4" :key="i" class="px-3 py-2.5 rounded-md space-y-1.5" :style="{ opacity: 1 - i * 0.2 }">
      ...
      <!-- Empty -->
      <div v-else-if="!pinnedMessages.length"
      ...
      <!-- No result -->
      <div v-else-if="!filtered.length" class="px-4 py-8 text-center text-[12px] text-muted-foreground">
      ...
      <div v-else class="flex flex-col px-2 gap-1">
        <div v-for="msg in filtered" :key="msg.id"
          ...
          <button
            ...
            :disabled="pinLoading" @click.stop="handleUnpin(msg.id)">
            Bỏ ghim
          </button>
        </div>
        <!-- Load more -->
        <button v-if="pinnedHasMore"
          ...
          :disabled="pinLoading" @click="loadMore">
          <div v-if="pinLoading" class="w-3 h-3 border-2 border-muted border-t-foreground rounded-full animate-spin" />
