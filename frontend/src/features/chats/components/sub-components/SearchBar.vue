...
<template>
  <div class="relative">
    ...
    <!-- Expanded -->
    <div v-else class="flex items-center gap-1.5 bg-muted rounded-md px-2.5 h-8">
      <Search class="w-3.5 h-3.5 text-foreground/70 shrink-0" />
      <input v-model="searchKeyword" placeholder="Tìm trong kênh..."
    ...
    <!-- Results Popover -->
    <div v-if="isSearching && searchActive" class="absolute top-full right-0 mt-2 w-96 z-50">
      <Card class="shadow-xl border overflow-hidden p-0">
        <CardContent class="p-0 max-h-72 overflow-y-auto">
          <div v-if="searchResults.length === 0" class="flex flex-col items-center justify-center py-8 gap-2">
            <Search class="w-8 h-8 text-foreground/70" />
            <p class="text-sm text-muted-foreground">Không tìm thấy kết quả</p>
          </div>

          <div v-else>
            <div v-for="msg in searchResults" :key="msg.id" @click="jumpToMessage(msg.id)"
              ...
              <Avatar class="w-7 h-7 shrink-0 mt-0.5">
                <AvatarImage v-if="msg.sender?.avatarUrl" :src="msg.sender.avatarUrl" />
                <AvatarFallback class="text-[10px] bg-primary text-primary-foreground">
                </AvatarFallback>
              </Avatar>
          ...
          <div v-if="searchHasMore" class="px-3 py-1.5 border-t bg-muted/30 justify-center">
            <button @click="loadMore" class="text-xs text-primary hover:underline font-medium">
              Xem thêm kết quả
            </button>
          </div>
        </CardContent>
